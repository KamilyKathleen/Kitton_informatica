...
def chama_configuração():   # Chama a tela de gerenciamento de produtos
    ...
    cursor = conexao.cursor()   # Função cursor, faz a conexao com a database
    cursor.execute("select * from produtos")    # Executa o comando select do MySQL
    ...
    kitton_configuracao.tableWidget.setColumnCount(25)  # Quantidade de linhas que aparecem de cada vez
...
def pesquisa_categoria():   # Pesquisa produtos pela categoria 
    ...
    cursor.execute("select * from produtos where categoria = '{}'".format(pesquisa))    # Executa o comando select do MySQL
    ...
    kitton_configuracao.tableWidget.setColumnCount(25)  # Quantidade de linhas que aparecem por vez
    for i in range(0, len(dados)):
        for j in range(0, 7):
            kitton_configuracao.tableWidget.setItem(i, j, QtWidgets.QTableWidgetItem(str(dados[i][j])))
...
def selecao_produto():
    ...
    cursor = conexao.cursor()   # Função cursor, faz a conexao com a database
    cursor.execute("select * from produtos")   # Executa o comando select codigo do MySQL
    dados = cursor.fetchall()   # Variável que busca/guarda todos os dados da database
    linha2 = dados[linha][0]
    cursor.execute("select * from produtos where codigo = '{}'".format(str(linha2)))    # Executa o comando select do MySQL
    selecionar = cursor.fetchall()  # Variável que busca/guarda todos os dados da database
    numero_id = linha2 
    kitton_configuracao.codigoPesquisa.setText(str(selecionar[0][0]))   # Mostra o código do produto
    kitton_configuracao.nomeAlterar.setText(str(selecionar[0][1]))  # Mostra o nome do produto
    kitton_configuracao.categoriaAlterar.setCurrentText(str(selecionar[0][2]))  # Mostra a categoria do produto
    kitton_configuracao.fornecedorAlterar.setText(str(selecionar[0][3]))    # Mostra o fornecedor do produto
    kitton_configuracao.custo_fabAlterar.setText(str(selecionar[0][4]))  # Mostra o custo de fábrica do produto
    kitton_configuracao.funcionarioAlterar.setCurrentText(str(selecionar[0][5]))    # Mostra o funcionário que cadastrou o produto
    kitton_configuracao.precoAlterar.setText(str(selecionar[0][6]))  # Mostra o preço do produto
...
def deleta_produtos():  # Deleta um registro na database
    ...
    dados = cursor.fetchall()   # Variável que busca/guarda todos os dados da database
    valor_cod = dados[linha][0] # Seleciona o código de acordo com a linha da tabela
...
def pesquisa_cod():     # Pesquisa o produto através do código dele
    cursor = conexao.cursor()   # Função cursor, faz a conexao com a database
    linha = kitton_caixa.inputCod.text()    # Armazena o código do produto
    cursor.execute("select codigo from produtos")   # Executa o comando select codigo do MySQL
    cursor.fetchall()   # Variável que busca/guarda todos os dados da database
    cursor.execute("select * from produtos where codigo = '{}'".format(str(linha))) # Executa o comando select do MySQL
    selecionar = cursor.fetchall()  # Variável que busca/guarda todos os dados da database
    kitton_caixa.inputCod.setText(str(selecionar[0][0]))    # Mostra o código do produto
    kitton_caixa.inputNome.setText(str(selecionar[0][1]))   # Mostra o nome do produto
    kitton_caixa.inputPreco.setText(str(selecionar[0][6]))  # Mostra o preço do produto
